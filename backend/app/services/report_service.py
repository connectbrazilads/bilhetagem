...
def dashboard_metrics(db: Session) -> dict:
    ...
    prints_today = db.query(func.count(PrintJob.id)).filter(authorized, PrintJob.submitted_at >= today_start).scalar() or 0
    prints_month = db.query(func.count(PrintJob.id)).filter(authorized, PrintJob.submitted_at >= month_start).scalar() or 0
