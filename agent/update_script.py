...
def build_self_update_script(
    ...
    lines = [
        ...
        'if errorlevel 1 echo [%date% %time%] Aviso ao parar servico; codigo=%ERRORLEVEL% >> "%LOG%"',
        "timeout /t 5 /nobreak > nul",
        'set "STAGE=criar backup"',
        'echo [%date% %time%] Criando backup do executavel atual >> "%LOG%"',
        f'copy /Y "{current_exe}" "{backup_path}" > nul',
        'if errorlevel 1 (echo [%date% %time%] Falha ao criar backup; codigo=%ERRORLEVEL% >> "%LOG%" & goto rollback)',
        'set "STAGE=substituir executavel"',
        'echo [%date% %time%] Aplicando nova versao >> "%LOG%"',
        f'copy /Y "{update_path}" "{current_exe}" > nul',
        'if errorlevel 1 (echo [%date% %time%] Falha ao substituir executavel; codigo=%ERRORLEVEL% >> "%LOG%" & goto rollback)',
        'set "STAGE=iniciar servico atualizado"',
        'echo [%date% %time%] Iniciando servico atualizado >> "%LOG%"',
        "sc start PrintBillingAgent > nul 2>&1",
        'if errorlevel 1 (echo [%date% %time%] Falha ao iniciar servico atualizado; codigo=%ERRORLEVEL% >> "%LOG%" & goto rollback)',
        ...
        'if errorlevel 1 echo [%date% %time%] Falha ao restaurar backup; codigo=%ERRORLEVEL% >> "%LOG%"',
        'echo [%date% %time%] Reiniciando servico apos rollback >> "%LOG%"',
        "sc start PrintBillingAgent > nul 2>&1",
        'if errorlevel 1 echo [%date% %time%] Falha ao iniciar servico apos rollback; codigo=%ERRORLEVEL% >> "%LOG%"',
        ":cleanup",
